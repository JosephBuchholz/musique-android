...
void Turn::Render(RenderData& renderData, float notePositionX, float topStaffLinePositionY, Vec2<float> offset) const
{
...
Vec2<float> Turn::GetDimensions() const
{
    BoundingBox bb = RenderMeasurement::GetGlyphBoundingBox(SMuFLGlyph(GetSMuFLID(), 0.0f, 0.0f, GetPaint()));
    return { bb.size.x, bb.size.y };
}

SMuFLID Turn::GetSMuFLID() const
{
    if (hasSlash)
        return SMuFLID::ornamentTurnSlash;

    switch (type)
    {
        case TurnType::Turn: return SMuFLID::ornamentTurn;
        case TurnType::DelayedTurn: return SMuFLID::ornamentTurn;
        case TurnType::InvertedTurn: return SMuFLID::ornamentTurnInverted;
        case TurnType::DelayedInvertedTurn: return SMuFLID::ornamentTurnInverted;
        case TurnType::VerticalTurn: return SMuFLID::ornamentTurnUp;
        case TurnType::InvertedVerticalTurn: return SMuFLID::ornamentTurnUpS;
        default: return SMuFLID::ErrorGlyph;
    }
}

Paint Turn::GetPaint() const
{
    Paint paint;
...
void Turn::UpdateBoundingBox(const Vec2<float>& parentPosition)
{
...
void Turn::CalculatePositionAsPaged(const MusicDisplayConstants& displayConstants, Vec2<float> defaultPosition)
{
