...
bool GetBoolAttribute(TiXmlElement* element, const char* s, bool defaultValue = false)
{
...
float GetFloatAttribute(TiXmlElement* element, const char* s, float defaultValue = 0.0f)
{
    const char* attribute = element->Attribute(s);
    if (attribute) {
...
std::string GetStringAttribute(TiXmlElement* element, const char* s, std::string defaultValue = "")
{
    const char* attribute = element->Attribute(s);
    if (attribute) {
        return attribute;
...
int GetNumberAttribute(TiXmlElement* element, const char* s, int defaultValue = 0)
{
    const char* attribute = element->Attribute(s);
    if (attribute) {
...
AboveBelowType GetAboveBelowAttribute(TiXmlElement* element, const char* s, AboveBelowType defaultValue = AboveBelowType::None)
{
    const char* attribute = element->Attribute(s);
    if (attribute) {
...
StartStopType GetStartStopAttribute(TiXmlElement* element, const char* s, StartStopType defaultValue = StartStopType::None)
{
    const char* attribute = element->Attribute(s);
    if (attribute) {
...
SongData* ParseMusicXML(const std::string& data, std::string& error)
{
    ...
    else
    {
        ...
        TiXmlElement* root = doc.FirstChildElement("score-partwise");
        if (root)
        {
            songData->musicXMLVersion = root->Attribute("version");

            // work
            TiXmlElement* work = root->FirstChildElement("work");
            if (work)
            {
                // work title
                TiXmlElement* workTitle = work->FirstChildElement("work-title");
            ...
            // part list
            TiXmlElement* partList = root->FirstChildElement("part-list");
            if (partList)
            {
                // score parts
                TiXmlNode* previousScorePartElement = partList->FirstChildElement("score-part");
                while (true)
                {
                    if (previousScorePartElement)
                    {
                        TiXmlElement* scorePart = previousScorePartElement->ToElement();
                        ...
                        // part name
                        TiXmlElement* partName = scorePart->FirstChildElement("part-name");
                        if (partName)
                        {
                        ...
                        TiXmlElement* partAbbreviation = scorePart->FirstChildElement("part-abbreviation");
                        if (partAbbreviation)
                        {
                        ...
                        // midi instrument
                        TiXmlElement* midiInstrumentElement = scorePart->FirstChildElement("midi-instrument");
                        if (midiInstrumentElement)
                        {
                            instrument->midiInstrument.id = GetStringAttribute(midiInstrumentElement, "id", "");

                            // midi channel
                            TiXmlElement* channelElement = midiInstrumentElement->FirstChildElement("midi-channel");
                            if (channelElement)
                            {
                            ...
                            // midi channel
                            TiXmlElement* programElement = midiInstrumentElement->FirstChildElement("midi-program");
                            if (programElement)
                            {
                            ...
                            // volume
                            TiXmlElement* volumeElement = midiInstrumentElement->FirstChildElement("volume");
                            if (volumeElement)
                            {
                            ...
                            // pan
                            TiXmlElement* panElement = midiInstrumentElement->FirstChildElement("pan");
                            if (panElement)
                            {
                    ...
                    previousScorePartElement = partList->IterateChildren("score-part", previousScorePartElement);
                }

                // old score part
                /*TiXmlElement* scorePart = partList->FirstChildElement("score-part");
                if (scorePart)
                {
                    Instrument* instrument = new Instrument();
                    songData->instruments.push_back(instrument);
            ...
            // parts
            TiXmlNode* previous = root->FirstChildElement("part");
            while (true) // looping through all parts
            {
                if (previous)
                {
                    TiXmlElement* part = previous->ToElement();
                    ...
                    */
                    ...
                    bool isTab = false;
                    TiXmlNode* previousMeasureElement = part->FirstChildElement("measure");
                    std::vector<Measure*> previousMeasures;
                    while (true)
                    {
                        if (previousMeasureElement)
                        {
                            TiXmlElement* measure = previousMeasureElement->ToElement();
                            ...
                            if (attributes)
                            {
                                // staves
                                int numStaves = 1;
                                TiXmlElement* stavesElement = attributes->FirstChildElement("staves");
                                if (stavesElement)
                                {
                                ...
                                // key signature
                                TiXmlElement* keySignatureElement = attributes->FirstChildElement("key");
                                if (keySignatureElement)
                                {
                                    ...
                                    // fifths
                                    TiXmlElement* fifths = keySignatureElement->FirstChildElement("fifths");
                                    if (fifths)
                                        keySignature.fifths = ToInt(fifths->GetText());

                                    // mode
                                    TiXmlElement* mode = keySignatureElement->FirstChildElement("mode");
                                    if (mode)
                                ...
                                // time signature
                                TiXmlElement* timeSignatureElement = attributes->FirstChildElement("time");
                                if (timeSignatureElement)
                                {
                                    ...
                                    TiXmlElement* beats = timeSignatureElement->FirstChildElement("beats");
                                    if (beats)
                                        timeSignature.notes = ToInt(beats->GetText());
                                    TiXmlElement* beatType = timeSignatureElement->FirstChildElement("beat-type");
                                    if (beatType)
                                ...
                                // clef
                                TiXmlNode* previousClefElement = attributes->FirstChildElement("clef");
                                if (previousClefElement)
                                {
                                    while (true)
                                    {
                                        if (previousClefElement)
                                        {
                                            TiXmlElement* clefElement = previousClefElement->ToElement();
                                            int clefNumber = GetNumberAttribute(clefElement, "number", 1);

                                            TiXmlElement* signElement = clefElement->FirstChildElement("sign");
                                            if (signElement)
                                            {
                                            ...
                                            TiXmlElement* line = clefElement->FirstChildElement("line");
                                            if (line)
                                        ...
                                        previousClefElement = attributes->IterateChildren("clef", previousClefElement);
                                    }
                                }

                                /*// clef
                                TiXmlElement* clef = attributes->FirstChildElement("clef");
                                if (clef)
                                {
                                    TiXmlElement* sign = clef->FirstChildElement("sign");
                                    if (sign)
                                    {
                                    ...
                                    TiXmlElement* line = clef->FirstChildElement("line");
                                    if (line)
                                ...
                                // transpose
                                TiXmlElement* transposeElement = attributes->FirstChildElement("transpose");
                                if (transposeElement)
                                {
                                    Transpose transpose = Transpose();
                                    // diatonic
                                    TiXmlElement* diatonic = transposeElement->FirstChildElement("diatonic");
                                    if (diatonic)
                                    {
                                    ...
                                    // chromatic
                                    TiXmlElement* chromatic = transposeElement->FirstChildElement("chromatic");
                                    if (chromatic)
                                    {
                                    ...
                                    // octave change
                                    TiXmlElement* octaveChange = transposeElement->FirstChildElement("octave-change");
                                    if (octaveChange)
                                    {
                                ...
                                // staff details
                                TiXmlElement* staffDetails = attributes->FirstChildElement("staff-details");
                                if (staffDetails)
                                {
                                    // staff lines
                                    TiXmlElement* staffLines = staffDetails->FirstChildElement("staff-lines");
                                    if (staffLines)
                                    ...
                                    if (currentInst->staves.back()->type == Staff::StaffType::Tab) {
                                        TiXmlNode *previousStaffTuning = staffDetails->FirstChildElement("staff-tuning");

                                        Staff* tabStaff = currentInst->staves.back();

                                        while (true) {
                                            if (previousStaffTuning) {
                                                TiXmlElement *staffTuning = previousStaffTuning->ToElement();
                                                TiXmlElement *tuningStep = staffTuning->FirstChildElement(
                                                        "tuning-step");
                                                StaffTuning tuning;
                                                if (tuningStep)
                                                    tuning.pitch.step = tuningStep->GetText();
                                                TiXmlElement *tuningOctave = staffTuning->FirstChildElement(
                                                        "tuning-octave");
                                                if (tuningOctave)
                                                    tuning.pitch.octave = ToInt(
                                                            tuningOctave->GetText());
                                                TiXmlElement *tuningAlter = staffTuning->FirstChildElement(
                                                        "tuning-alter");
                                                if (tuningAlter)
                                            ...
                                            previousStaffTuning = staffDetails->IterateChildren(
                                                    "staff-tuning", previousStaffTuning);
                                        }
                                    }
                                }
                            }
                            ...
                            TiXmlNode* previousElement = measure->FirstChildElement(); // the first element
                            //TiXmlNode* previousNoteElement = measure->FirstChildElement("note");
                            Note* previousNote = nullptr;
                            while (true)
                            {
                                if (previousElement)
                                {
                                    TiXmlElement* element = previousElement->ToElement();
                                    const char* value = element->Value();
                                    if (strcmp(value, "note") == 0) // note
                                    {
                                        TiXmlElement* noteElement = element;
                                        ...
                                        // staff
                                        TiXmlElement* staffElement = noteElement->FirstChildElement("staff");
                                        if (staffElement)
                                        {
                                        ...
                                        // chord
                                        TiXmlElement* chordElement = noteElement->FirstChildElement("chord");
                                        if (chordElement)
                                        {
                                        ...
                                        // rest
                                        TiXmlElement* restElement = noteElement->FirstChildElement("rest");
                                        if (restElement)
                                        {
                                        ...
                                        // pitch
                                        TiXmlElement* pitch = noteElement->FirstChildElement("pitch");
                                        if (pitch)
                                        {
                                            // step
                                        ...
                                        // duration
                                        TiXmlElement* duration = noteElement->FirstChildElement("duration");
                                        if (duration)
                                        {
                                        ...
                                        // tie
                                        TiXmlElement* tieElement = noteElement->FirstChildElement("tie");
                                        if (tieElement)
                                        {
                                        ...
                                        // voice
                                        TiXmlElement* voiceElement = noteElement->FirstChildElement("voice");
                                        if (voiceElement)
                                        {
                                        ...
                                        // type
                                        TiXmlElement* noteType = noteElement->FirstChildElement("type");
                                        if (noteType)
                                        {
                                        ...
                                        // accidental
                                        TiXmlElement* accidentalElement = noteElement->FirstChildElement("accidental");
                                        ...
                                        // stem
                                        TiXmlElement* stemElement = noteElement->FirstChildElement("stem");
                                        if (stemElement)
                                        {
                                        ...
                                        // beam
                                        TiXmlElement* beamElement = noteElement->FirstChildElement("beam");
                                        if (beamElement)
                                        {
                                        ...
                                        {
                                            // slurs
                                            TiXmlElement* slurElement = notations->FirstChildElement("slur");
                                            if (slurElement)
                                            {
                                                Slur slur = Slur();
                                            ...
                                            {
                                                // hammer ons
                                                TiXmlNode* previousHammerOnElement = technical->FirstChildElement("hammer-on");
                                                while (true)
                                                {
                                                    if (previousHammerOnElement) {
                                                        TiXmlElement* hammerOnElement = previousHammerOnElement->ToElement();
                                                        if (hammerOnElement)
                                                        {
                                                    ...
                                                    previousHammerOnElement = technical->IterateChildren("hammer-on", previousHammerOnElement);
                                                }

                                                // pull offs
                                                TiXmlNode* previousPullOffElement = technical->FirstChildElement("pull-off");
                                                while (true)
                                                {
                                                    if (previousPullOffElement) {
                                                        TiXmlElement* pullOffElement = previousPullOffElement->ToElement();
                                                        if (pullOffElement)
                                                        {
                                                            TABSlur tabSlur = TABSlur();
                                                    ...
                                                    previousPullOffElement = technical->IterateChildren("pull-off", previousPullOffElement);
                                                }
                                                ...
                                                if (currentNote->type == Note::NoteType::Tab and isTab) {
                                                    ...
                                                    // string
                                                    TiXmlElement* string = technical->FirstChildElement("string");
                                                    if (string)
                                                    {
                                                    ...
                                                    // fret
                                                    TiXmlElement* fret = technical->FirstChildElement("fret");
                                                    if (fret)
                                                    {
                                    ...
                                    {
                                        // duration
                                        TiXmlElement* durationElement = element->FirstChildElement("duration");
                                        if (durationElement)
                                        {
                                    ...
                                    {
                                        // sound
                                        TiXmlElement* soundElement = element->FirstChildElement("sound");
                                        if (soundElement)
                                        {
                                ...
                                previousElement = measure->IterateChildren(previousElement); // get the next element
                        ...
                        previousMeasureElement = part->IterateChildren("measure", previousMeasureElement);
                    }
                ...
                previous = root->IterateChildren("part", previous);
            }
        }
        else
        {
